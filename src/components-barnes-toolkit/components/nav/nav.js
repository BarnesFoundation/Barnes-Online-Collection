A17.Behaviors.nav = function(container) {
  ...
  /**
   * @var {object}
   * An object to house our touch in our touch-wipe.js class
   */
  var _touchObject;
   ...
   */
  var _cachedElements = {
    showButtons: document.querySelectorAll('[data-nav-show]'),
    hideButtons: document.querySelectorAll('[data-nav-hide]'),
    ...
    otherElements: document.querySelectorAll('header, .m-search-bar, main, footer, .skip-link'),
  ...
  /**
   * _focusFirstNavElement
   * Find the first focussable element in the active nav and
   * set focus to it for accessibility. If the user has clicked the search icon
   * then focus the nav instead
   * @param {object} currentTarget
   *
   * @return {void}
   */
  function _focusFirstNavElement(currentTarget) {

    var firstFocusableEl = _focusableEls[0];

    if (currentTarget.getAttribute('aria-labelledby') == 'search-open-title') {
  ...
  /**
   * _swipeClose
   * use the touch-wipe plugin to detect a swipe from left to right
   *
   * @return {void}
   */
  function _swipeClose() {

    _touchObject = touchwipe(container, {
      wipeRight: function() {
        _hide();
      },
      min_move_x: 50,
  ...
  function _show(e) {
    ...
    // Set focus to the nav - so the user can tab through the
    // Reasons = Browsers :/
    // 1 only do this after the transition ends or Safari freaks out
    // 2 remove the eventListener otherwise IE11 goes nuts
    container.addEventListener('transitionend', transitionEnd = function(event) {
      container.removeEventListener('transitionend', transitionEnd);
      _focusFirstNavElement(currentTarget);
    }, {once: true });
  ...
  /**
   * _safari9Fix
   * test to see if we are on Safari 8 or 9 and then remove the transition from
   * the container as this causes a weird bug on close where entire page jumps
   * left after _hide() runs
   *
   * @return {void}
   */
  function _safari9Fix() {
    ...
    if(isSafari) {
      isSafari9 = navigator.userAgent.match(/Version\/9/);
      isSafari8 = navigator.userAgent.match(/Version\/8/);

      if (isSafari9 || isSafari8) {
