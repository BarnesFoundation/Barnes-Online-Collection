...
class SiteHeader extends Component {
  ...
  keyListener = (e) => {
    const { isSideMenuOpen, isHeaderHidden } = this.state;
    const { isGlobalSearchHeader, toggleGlobalSearch, isGlobalSearchActive } =
      this.props;
    ...
    if (
      ...
      !isSideMenuOpen &&
      (isHeaderHidden === HEADER_HIDDEN.DEFAULT ||
        isHeaderHidden === HEADER_HIDDEN.LOCKED) &&
      this.startRef &&
      this.endRef &&
  ...
  render() {
    ...
    if (isArtObjectClassNames && isSecond)
      isArtObjectClassNames = `${isArtObjectClassNames} art-object-header--absolute`; // For second menu on artist page, absolutely position second menu.
    ...
    if (
      (!isGlobalSearchHeader && isHeaderHidden === HEADER_HIDDEN.UNLOCKED) ||
      (isGlobalSearchHeader && !isGlobalSearchActive)
    ) {
    ...
    if (!isGlobalSearchActive)
      gHeaderNavLinkClassNames = `${gHeaderNavLinkClassNames} g-header__nav__link--active`;

    let gHeaderBtnClassNames =
      "g-header__nav__btn g-header__btn__search btn btn--icon-only html4-hidden";
    if (isGlobalSearchActive)
      gHeaderBtnClassNames = `${gHeaderBtnClassNames} g-header__nav__btn--active`;
            ...
            <div className="global-search">
              {isGlobalSearchActive && (
                <SearchBar
                  ...
                  submit={(query) =>
                    (window.location.href = `${MAIN_WEBSITE_DOMAIN}/search?q=${query}`)
                  }
                  tabIndex={tabIndex}
                />
              )}
              <div className="container global-search__buttons-area">
                <span className="global-search__buttons-term">
                  Suggested terms
                </span>
                  ...
                  {SUGGESTED_TERMS.map((term, i) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={(e) =>
                          (window.location.href = `${MAIN_WEBSITE_DOMAIN}/search?q=${term.toLowerCase()}`)
                        }
                        tabIndex={tabIndex}
                        {...additionalProps}
                      >
...
const mapStateToProps = (state) => ({
  isArtistMenuToggled: state.filterSets.isArtistMenuToggled,
});
...
class SiteHeaderGlobalSearch extends Component {
  ...
  render() {
    ...
    if (isGlobalSearchActive)
      shadedOverlayClassNames = `${shadedOverlayClassNames} shaded-background__tint--active`;
    if (overlayActive)
    ...
    return (
      <div>
        <ConnectedSiteHeader
          ...
          toggleGlobalSearch={() =>
            this.setGlobalSearchStatus(!isGlobalSearchActive)
          }
