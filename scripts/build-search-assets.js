 ...
 */
const getUniqueSearchValues = async (
  aggregationName,
  aggregationField,
  subAggregation
) => {
  // Add the sub-aggregation query if subAggregation defined
  const nest = subAggregation
    ? (a) =>
        a.aggregation(
          "top_hits",
          { size: 1, _source: [subAggregation] },
          subAggregation
        )
    : null;
      ...
      (error, response) => {
        ...
        if (subAggregation) {
          buckets = buckets.reduce((acc, cv) => {
            ...
            const field =
              cv[subAggregation]["hits"]["hits"][0]._source[subAggregation];
...
const writeAssetsFile = async (fileName, fileContents) => {
  return await new Promise((resolve) => {
    fs.writeFile(
      path.join(constantsDirectory, fileName),
      fileContents,
      (error) => {
        if (error) {
          console.log(error);
          ...
          resolve("Successfully wrote searchAssets.json");
        }
      }
    );
  });
...
const generateAssets = async () => {
  const searchAssetsObject = {
    ...
    artists: (
      await getUniqueSearchValues("uniq_peoples", "people.text", "sortedName")
    ).map(({ sortedName, key, ...rest }) => ({
      sortedName: fixDiacritics(sortedName),
      key: fixDiacritics(key),
      ...rest,
    })),
    ...
    classifications: await getUniqueSearchValues(
      "uniq_classifications",
      "classification"
    ),
...
const generateAndWriteAssets = async () => {
  ...
  const result = await writeAssetsFile(
    "searchAssets.json",
