...
const META_TITLE =
  process.env.REACT_APP_META_TITLE || "Barnes Collection Online";
...
const DEFAULT_TITLE_URL =
  process.env.DEFAULT_TITLE_URL || "barnes-collection-object";
...
const normalizeDissimilarPercent = (req, res, next) => {
  if (req.query.dissimilarPercent !== undefined) {
    req.query.dissimilarPercent =
      Math.round(req.query.dissimilarPercent / 10) * 10;
  }
  next();
};
  ...
  if (req.query.cache === "false") {
    ...
    const cacheKey = (objectID, dissimilarPercent) => {
      if (objectID === undefined) {
        throw new Error(`objectID undefined`);
      }
...
const generateObjectImageUrls = (object) => {
  ...
  const IMAGE_BASE_URL =
    process.env.REACT_APP_IMAGE_BASE_URL || `//s3.amazonaws.com/${AWS_BUCKET}`;
...
const getDistance = (from, to) => {
  ...
  const distance = distanceKeys.reduce((sum, key) => {
    ...
    switch (key) {
      case "people":
        absoluteDistance = from[key] === to[key] ? 0 : 100;
        break;
      default:
        absoluteDistance = parseFloat(to[key]) - parseFloat(from[key]);
        break;
    }
...
app.get("/objects/:id/:title/:panel", (req, res, next) => {
  ...
  if (titleSlug !== req.params.title) {
    // redirect to the correct title
    const redirectUrl =
      getCorrectTitleUrl(req.params.id, titleSlug) + `${req.params.panel}`;
    return res.redirect(301, redirectUrl);
  }
